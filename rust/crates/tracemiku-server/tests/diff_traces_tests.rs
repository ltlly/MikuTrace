...
fn make_trace_dir(root: &Path, name: &str, x_sign: &[u8]) -> PathBuf {
    ...
    let events = [
        json!({"id":"NewStringUTF","trace_idx":1,"args":{"bytes":"x-sign"}}),
...
async fn diff_traces_reports_stable_and_variable_offsets() {
    ...
    let run1 = make_trace_dir(tmp.path(), "run1", &hex_bytes("6b360108aaaaaaaa11111111"));
    let run2 = make_trace_dir(tmp.path(), "run2", &hex_bytes("6b360108bbbbbbbb11111111"));
    let body = json!({
    ...
    let xsign = &v["headers"]["x-sign"];
    assert_eq!(xsign["len_compared"], 12);
    assert_eq!(xsign["stable_count"], 8);
    assert_eq!(xsign["variable_count"], 4);
    assert_eq!(xsign["stable_offsets"], json!([0, 1, 2, 3, 8, 9, 10, 11]));
    assert_eq!(xsign["variable_offsets"], json!([4, 5, 6, 7]));
}

#[tokio::test]
async fn diff_traces_requires_two_traces() {
    let tmp = tempfile::tempdir().unwrap();
    let run1 = make_trace_dir(tmp.path(), "run1", b"AAAAAAAA");
