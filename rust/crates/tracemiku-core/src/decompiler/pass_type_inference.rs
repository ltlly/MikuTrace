...
impl TypePropagationPass {
    fn walk_operand_vars(
        ...
        match op {
            PassIlOperand::Var(name) => {
                cb(name, parent_op, operand_idx, sibling_ops);
            }
    ...
    fn infer_from_context(
        ...
        context_op: &str,
        operand_idx: usize,
        siblings: &[PassIlOperand],
        known_types: &BTreeMap<String, InferredType>,
    ) -> InferredType {
        match context_op {
            ...
            | "HLIL_Or" | "LLIL_Xor" | "MLIL_Xor" | "HLIL_Xor" | "LLIL_Neg" | "MLIL_Neg"
            | "HLIL_Neg" | "LLIL_Lsl" | "MLIL_Lsl" | "HLIL_Lsl" | "LLIL_Lsr" | "MLIL_Lsr"
            | "HLIL_Lsr" | "LLIL_Asr" | "MLIL_Asr" | "HLIL_Asr" | "LLIL_CmpE" | "MLIL_CmpE"
            | "LLIL_CmpNe" | "MLIL_CmpNe" | "LLIL_CmpSlt" | "LLIL_CmpSle" | "LLIL_CmpSgt"
            | "LLIL_CmpSge" | "LLIL_CmpUlt" | "LLIL_CmpUle" | "LLIL_CmpUgt" | "LLIL_CmpUge" => {
                InferredType::Int
            }
    ...
    fn collect_max_type(
        ...
        max_ty: &mut InferredType,
    ) {
        match op {
            PassIlOperand::Var(name) => {
                if let Some(&ty) = types.get(name) {
                    *max_ty = max_ty.merge(ty);
                }
            }
            PassIlOperand::Expr(e) => {
                for child in &e.operands {
                    Self::collect_max_type(child, types, max_ty);
                }
            }
            _ => {}
        }
                ...
                "LLIL_SetReg" | "MLIL_SetVar" | "HLIL_SetVar" => {
                    ...
                    if let PassIlOperand::Var(dest) = &e.operands[0] {
                        if let Some(&ty) = types.get(dest) {
                            if ty != InferredType::Unknown {
                                let already =
                                    e.extra.iter().any(|(k, v)| k == "type" && v == ty.as_str());
                                if !already {
...
impl Pass for TypePropagationPass {
    fn info(&self) -> PassInfo {
        PassInfo {
            ...
            description:
                "Infer variable types (Ptr/Uint/Sint/Int) from usage patterns and propagate forward",
            phase: 1,
            requires: &[],
            invalidates: &[],
    ...
    fn run(&self, _ctx: &PassContext, exprs: &mut PassIlExprs) -> PassResult {
        ...
        for _ in 0..10 {
            if !Self::propagate_through_assignments(&exprs.exprs, &mut types) {
                break;
            }
        }
        if Self::annotate_types(&mut exprs.exprs, &types) {
            PassResult::Changed
        } else {
...
mod tests {
    ...
    fn test_infer_ptr_from_load() {
        ...
        let has_ptr = exprs.exprs[0]
            .extra
            .iter()
            .any(|(k, v)| k == "type" && v == "ptr");
    ...
    fn test_infer_uint_from_zext() {
        ...
        let result = pass.run(&ctx, &mut exprs);
        assert!(result.is_changed());
        let has_uint = exprs.exprs[0]
            .extra
            .iter()
            .any(|(k, v)| k == "type" && v == "uint");
    ...
    fn test_infer_sint_from_sext() {
        ...
        let result = pass.run(&ctx, &mut exprs);
        assert!(result.is_changed());
        let has_sint = exprs.exprs[0]
            .extra
            .iter()
            .any(|(k, v)| k == "type" && v == "sint");
    ...
    fn test_infer_int_from_arithmetic() {
        ...
        let result = pass.run(&ctx, &mut exprs);
        assert!(result.is_changed());
        let has_int = exprs.exprs[0]
            .extra
            .iter()
            .any(|(k, v)| k == "type" && v == "int");
