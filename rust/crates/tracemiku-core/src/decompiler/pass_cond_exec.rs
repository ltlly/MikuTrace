...
impl ConditionalExecutionPass {
    ...
    fn annotate_csel(expr: &mut PassIlExpr) -> bool {
        ...
        expr.extra.push(("cond_expr".to_string(), "csel".to_string()));
    ...
    fn describe_condition(cond_op: &PassIlOperand) -> Option<String> {
        match cond_op {
            PassIlOperand::Var(name) => {
                if name.contains("z") { Some("eq".to_string()) }
                else if name.contains("c") { Some("cs".to_string()) }
                else if name.contains("n") { Some("mi".to_string()) }
                else if name.contains("v") { Some("vs".to_string()) }
                else { Some(format!("flag({})", name)) }
            }
    ...
    fn annotate_csel_recursive(expr: &mut PassIlExpr) -> bool {
        ...
        if Self::is_csel(expr) {
            if Self::annotate_csel(expr) { changed = true; }
        }
        for op in &mut expr.operands {
            if let PassIlOperand::Expr(ref mut child) = op {
                if Self::annotate_csel_recursive(child) { changed = true; }
            }
        }
        changed
    }
    ...
    fn fold_if_else_patterns(exprs: &mut Vec<PassIlExpr>) -> bool {
        ...
        while i < exprs.len() {
            ...
            if expr.op == "LLIL_If" || expr.op == "MLIL_If" {
                if expr.operands.len() < 3 { i += 1; continue; }
                ...
                let true_setreg = exprs.iter().position(|e| {
                    e.pc == true_pc && (e.op == "LLIL_SetReg" || e.op == "MLIL_SetVar" || e.op == "HLIL_SetVar")
                });
                let false_setreg = exprs.iter().position(|e| {
                    e.pc == false_pc && (e.op == "LLIL_SetReg" || e.op == "MLIL_SetVar" || e.op == "HLIL_SetVar")
                });
                if let (Some(t_idx), Some(f_idx)) = (true_setreg, false_setreg) {
                    ...
                    if t_dest == f_dest {
                        let true_val = exprs[t_idx].operands.get(1).cloned()
                            .unwrap_or(PassIlOperand::Var(t_dest.clone()));
                        let false_val = exprs[f_idx].operands.get(1).cloned()
                            .unwrap_or(PassIlOperand::Var(t_dest.clone()));
                        let csel_expr = PassIlExpr {
                            op: "LLIL_Csel".to_string(), size: exprs[t_idx].size, pc: exprs[i].pc,
                            operands: vec![cond, true_val, false_val],
                            extra: vec![("cond_expr".to_string(), "ifelse_folded".to_string())],
                        };
                        exprs[i] = PassIlExpr {
                            op: "LLIL_SetReg".to_string(), size: exprs[t_idx].size, pc: exprs[i].pc,
                            operands: vec![PassIlOperand::Var(t_dest), PassIlOperand::Expr(Box::new(csel_expr))],
                            extra: vec![("cond_expr".to_string(), "ifelse_folded".to_string())],
                        };
                        if t_idx > i { exprs[t_idx].extra.push(("folded_cond".to_string(), "dead".to_string())); }
                        if f_idx > i { exprs[f_idx].extra.push(("folded_cond".to_string(), "dead".to_string())); }
                        changed = true;
                    }
                }
            }
        ...
        if changed {
            exprs.retain(|e| !e.extra.iter().any(|(k, v)| k == "folded_cond" && v == "dead"));
        }
        changed
    }
}
...
impl Pass for ConditionalExecutionPass {
    ...
    fn run(&self, _ctx: &PassContext, exprs: &mut PassIlExprs) -> PassResult {
        ...
        for e in &mut exprs.exprs {
            if Self::annotate_csel_recursive(e) { changed = true; }
        }
        if Self::fold_if_else_patterns(&mut exprs.exprs) { changed = true; }
...
mod tests {
    ...
    fn test_annotate_csel() {
        ...
        exprs.exprs = vec![
            make_expr("LLIL_SetReg", vec![
                reg("x0#1"),
                PassIlOperand::Expr(Box::new(make_expr("LLIL_Csel", vec![reg("z"), reg("x1#1"), reg("x2#1")]))),
            ]),
        ];
        ...
        if let PassIlOperand::Expr(ref csel) = exprs.exprs[0].operands[1] {
            assert!(csel.extra.iter().any(|(k, _)| k == "cond_expr"), "Csel should have cond_expr");
        } else { panic!("expected Csel expr"); }
    }
    ...
    fn test_fold_if_else_to_csel() {
        let mut exprs = PassIlExprs::new("test", "llil");
        exprs.exprs = vec![
            make_expr_at("LLIL_If", vec![reg("z"), u64val(0x2000), u64val(0x3000)], 0x1000),
        ...
        } else { panic!("expected Csel in folded result"); }
    }
    ...
    fn test_csel_with_compare_condition() {
        let mut exprs = PassIlExprs::new("test", "llil");
        let cmp = PassIlOperand::Expr(Box::new(make_expr("LLIL_CmpE", vec![reg("x0#1"), reg("x1#1")])));
        exprs.exprs = vec![
            make_expr("LLIL_SetReg", vec![
                reg("x2#1"),
                PassIlOperand::Expr(Box::new(make_expr("LLIL_Csel", vec![cmp, reg("x3#1"), reg("x4#1")]))),
            ]),
        ];
        let pass = ConditionalExecutionPass;
        let ctx = PassContext { function_name: "test", phase: 1, verbose: false };
        ...
        assert!(result.is_changed());
        if let PassIlOperand::Expr(ref csel) = exprs.exprs[0].operands[1] {
            let has_eq = csel.extra.iter().any(|(k, v)| k == "cond_type" && v == "eq");
            assert!(has_eq, "Csel with CmpE should have cond_type=eq, got {:?}", csel.extra);
        }
    }
}
