...
fn make_fingerprint(name: &str, category: Category, alg: &str, value: u32, mask: u32) -> Fingerprint {
    Fingerprint { name: name.to_string(), category, alg: alg.to_string(), value, mask }
}
...
pub fn build_fingerprints() -> Vec<Fingerprint> {
    ...
    for (i, &v) in sha256_iv.iter().enumerate() {
        fps.push(exact(&format!("SHA256.IV{}", i), Category::Hash, "SHA-256", v));
    }
    ...
    for (i, &k) in sha256_k.iter().enumerate() {
        fps.push(exact(&format!("SHA256.K[{}]", i), Category::Hash, "SHA-256", k));
    }
    ...
    for (i, &rc) in sha3_rc.iter().enumerate() {
        ...
        fps.push(exact(&format!("SHA3.RC[{}]", i), Category::Hash, "SHA-3", rc as u32));
    }
    ...
    for (i, &fk) in sm4_fk.iter().enumerate() {
        fps.push(exact(&format!("SM4.FK[{}]", i), Category::SymCipher, "SM4", fk));
    }
    ...
    // === ChaCha20 ===
    fps.push(exact("ChaCha20.expand0", Category::SymCipher, "ChaCha20", 0x61707865));
    fps.push(exact("ChaCha20.expand1", Category::SymCipher, "ChaCha20", 0x3320646e));
    fps.push(exact("ChaCha20.expand2", Category::SymCipher, "ChaCha20", 0x79622d32));
    fps.push(exact("ChaCha20.expand3", Category::SymCipher, "ChaCha20", 0x6b206574));

    // === Poly1305 ===
    fps.push(exact("Poly1305.r_mask", Category::Mac, "Poly1305", 0x0ffffffc));
    ...
    fps.push(exact("secp256k1.p_lo", Category::Ecc, "secp256k1", 0xfffffc2f));
    // Curve25519 p = 2^255 - 19
    fps.push(exact("Curve25519.p_lo", Category::Ecc, "Curve25519", 0xffffffed));
...
const CRYPTO_INSTRS: &[(&str, &str)] = &[
    // AES
    ("aese", "AES"), ("aesmc", "AES"), ("aesd", "AES"), ("aesimc", "AES"),
    // SHA-1
    ("sha1c", "SHA-1"), ("sha1m", "SHA-1"), ("sha1p", "SHA-1"),
    ("sha1h", "SHA-1"), ("sha1su0", "SHA-1"), ("sha1su1", "SHA-1"),
    // SHA-256
    ("sha256h", "SHA-256"), ("sha256h2", "SHA-256"),
    ("sha256su0", "SHA-256"), ("sha256su1", "SHA-256"),
    // SHA-512
    ("sha512h", "SHA-512"), ("sha512h2", "SHA-512"),
    ("sha512su0", "SHA-512"), ("sha512su1", "SHA-512"),
    // SHA-3
    ("eor3", "SHA-3"), ("rax1", "SHA-3"), ("xar", "SHA-3"), ("bcax", "SHA-3"),
    // GHASH / PMULL
    ("pmull", "GHASH"), ("pmull2", "GHASH"),
    // SM3
    ("sm3ss1", "SM3"), ("sm3tt1a", "SM3"), ("sm3tt1b", "SM3"),
    ("sm3partw1", "SM3"), ("sm3partw2", "SM3"),
    // SM4
    ("sm4e", "SM4"), ("sm4ekey", "SM4"),
];
...
pub fn scan_combined(trace: &Trace) -> (ConstScanResult, CryptoInstrResult) {
                ...
                if let Ok(v) = u64::from_str_radix(cleaned, 16) {
                    ...
                    if !name.is_empty() {
                        let hit = ConstHit {
                            ...
                            category: Category::Mac, alg: "HMAC".to_string(),
                            idx: i, pc: rec.pc, source: ConstHitSource::Imm,
                            verdict: Verdict::RealSimd, sample_value: v as u32,
                        };
    ...
    (const_result, CryptoInstrResult {
        hits: instr_hits,
        records_scanned: trace.len(),
    })
}
...
mod tests {
    ...
    fn fingerprint_count() {
        ...
        assert!(fps.len() >= 90, "expected at least 90 fingerprints, got {}", fps.len());
    }
