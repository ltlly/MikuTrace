...
rust/target/debug/tracemiku-cli byte-lineage <call_dir> --addr <addr> --before-idx <idx> --count <n> --compact
```
...
| `byte-lineage --compact` | Minimal byte provenance digest with path, recognized semantics, compact formula operands, memory boundaries, and next actions. Boundary rows include `mem_dump_command` hints with the correct `--cursor` for checking observed bytes. Add `--count N` to batch consecutive bytes without rebuilding indexes for every byte; batch output includes `decision_counts`, `upstream_counts`, `error_count`, and `step_stats`. Use it before requesting the full chain. |
| `byte-lineage --compact --count N` `frontier_groups[]` | AI-facing grouping for consecutive bytes. It clusters bytes by terminal decision and upstream boundary, reports offset ranges, step stats, repeated values, stable pointer loops, terminal addresses, observed boundary bytes, and a generic next action. Use this to split a long output buffer into proof tasks such as bytecode-literal lifting, pre-trace table parameterization, or trace-widening, instead of reading every byte chain manually. |
...
| `local_def.syscall_return` | Explicit syscall-return boundary for `x0` values after `svc`. It records the `svc` idx, `x8` syscall number, return value, argument registers, and skipped non-def rows. Treat outputs such as `getpid()` as external parameters unless the trace also captures the kernel-side source. |
...
| `stable_pointer_loop` | Emitted by `byte-lineage --compact` when a depth/cycle stop is dominated by one repeated pointer-shaped value. Treat it as a copy/base boundary and prove that pointer once; increasing depth usually wastes time. |
...
| `bytecode-read` frontier | The trace reached VM bytecode or an immediate. `byte-lineage` treats this as `bytecode_read_boundary` and stops instead of chasing the bytecode pointer register. This is a good stopping point for opcode-template lifting. |
