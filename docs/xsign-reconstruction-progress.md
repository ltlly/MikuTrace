...
The fourth suffix word also follows that frontier pattern:
...
includes `decision_counts`, `upstream_counts`, `error_count`, `step_stats`, and
`frontier_groups`, so agents can pick frontier classes and contiguous proof
ranges without a follow-up `jq` reduction.
