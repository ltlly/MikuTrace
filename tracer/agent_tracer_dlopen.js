// Variant: hook android_dlopen_ext to catch libsgmainso load *before*
// JNI_OnLoad runs. Tests whether TB detects the dlopen Interceptor.
const REC_SIZE = 272;
        ...
        try {
            Interceptor.attach(p, {
                ...
                onLeave(retv) {
                    ...
                    // Arm IMMEDIATELY in onLeave context — JNI_OnLoad hasn't been called yet
                    armOnTarget();
                }
            });
            STATE.dlopenHooks++;
...
rpc.exports = {
    init(opts) {
        ...
        // Try to hook dlopen FIRST (cheap, modifies one or two libc symbols)
        try { installDlopenHooks(); }
        catch (e) { log("[!] installDlopenHooks failed: " + e); }
        // If already loaded, arm immediately
        if (armOnTarget()) {
