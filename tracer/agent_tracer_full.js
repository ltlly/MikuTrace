// Variant of agent_tracer_dlopen.js but WITHOUT PC filter — record EVERY
// instruction Stalker sees on the followed thread (within libsgmainso AND
// outside, so libart/libc calls show up too).
// This lets us see why traces previously "ended" at ~4976 records: was it
// because execution went into libart and stayed (filter cut us off), or
// because the process was killed?
//
// CAUTION: full-trace mode generates many MB/s. Use only briefly.
const REC_SIZE = 272;
const BATCH_RECS = 4096;
const BATCH_BYTES = REC_SIZE * BATCH_RECS;
const FLUSH_INTERVAL_MS = 200;
const MAX_RECORDS = 200000; // safety cap to avoid OOM/disk fill
...
function recordInsn(ctx) {
    ...
    if (STATE.totalRecs + STATE.batchOff/REC_SIZE >= MAX_RECORDS) {
        STATE.capped = true;
        log(`[!] cap reached at ${MAX_RECORDS} records — stopping`);
        flush("cap");
...
function startFollow(tid) {
    ...
    log(`[+] Stalker.follow tid=${tid} (NO FILTER, full trace)`);
}
