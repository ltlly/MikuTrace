...
TRACE_CALL001_SCRATCH_TABLE_WRITER_CHAIN_SUMMARY = {
    ...
    "middle_lhs_trace_produced_table_word_probe": {
        ...
        "status": "ready",
        "refined_source_class": "trace_produced_table_word_pending_upstream",
        "writer": {
            "idx": 14164461,
            "asm": "str w16, [x2, x5]",
            "addr": "0x74b68bbe08",
        ...
        "interpretation": (
            ...
            "earlier traced memory writer, so the previous static/no-writer "
            "classification was a window artifact for this segment. The "
            "portable model still needs the upstream source for #14019868."
        ),
    },
    "interpretation": (
        "The scratch lhs table is generated by mixed VM writer chains. The "
        "first two runs consume stat('/').st_mtim.tv_sec, the 8..12 run "
        "is now known to be trace-produced by an orr/ubfx/lsr ladder that "
        "still needs its earlier memory writer classified, the 12..36 runs "
...
def parameterized_simulation_contract() -> dict:
    return {
        ...
        "opaque_or_nonportable_inputs": [
            {
                "name": "middle_lhs_trace_produced_table_word",
                "semantic_range": [21, 25],
                "refined_by": (
                    "TRACE_CALL001_SCRATCH_TABLE_WRITER_CHAIN_SUMMARY."
                    "middle_lhs_trace_produced_table_word_probe"
                ...
                "goal": (
                    "Classify the upstream #14019868 writer for 0x95f2ec79 "
                    "as a portable formula, VM table value, or external input."
                ),
            },
            {
